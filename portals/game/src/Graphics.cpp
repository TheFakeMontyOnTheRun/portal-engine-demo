...
void graphicsEncloseClipRect( int16_t x0, int16_t y0, int16_t x1, int16_t y1 ) {
  ...
  clippingRect.y1 = std::min(y1, clippingRect.y1);
   ...
   /*
     ...
     *         \| x1y1
     */
void graphicsDrawWall( FixP x0, FixP x1, FixP x0y0, FixP x0y1, FixP x1y0, FixP x1y1, NativeBitmap* texture, FixP mx, FixP my ) {
  ...
  for (; ix < limit; ++ix ) {
     ...
     */
void graphicsDrawFloor(FixP y0, FixP y1, FixP x0y0, FixP x1y0, FixP x0y1, FixP x1y1, NativeBitmap* texture, FixP mx, FixP my ) {
...
void graphicsDrawSprite( FixP x0, FixP y0, FixP x1, FixP y1, NativeBitmap* texture, bool useAlpha, FixP mx, FixP my ) {
  ...
  for (; iy < limit; ++iy ) {
    ...
    if (iy < clippingRect.y1 && iy >= clippingRect.y0) {
      TextureFixP u{0};
      const uint8_t iv = static_cast<uint8_t >(v);
      uint8_t* sourceLineStart = data + ((iv & (textureHeight - 1)) * textureWidth);
      uint8_t* destinationLine = bufferData + (320 * iy) + iX0;
...
void graphicsDrawTextAt( int16_t x, int16_t y, const char* text, uint8_t colour ) {
  ...
  for ( size_t c = 0; c < len; ++c ) {
    uint8_t ascii = text[c] - ' '; //the font starts with space, thus it's interesting to base the ascii code from there.
    ...
    for ( uint_fast8_t srcY = 0; srcY < 8; ++srcY ) {
      ...
      uint8_t* letterSrc = letter + ( fontWidth * srcY ); //skip the remaining font line to read the next line of the font
...
void graphicsBlit( int16_t dx, int16_t dy, NativeBitmap* src) {
  ...
  for ( size_t y = 0; y < height; ++y ) {
    ...
    uint8_t* sourceLineStart = sourceLine + ( src->width * (offsetY + y) ) + offsetX;
